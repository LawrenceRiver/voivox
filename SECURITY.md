...
- Native Messaging is restricted to VOIVOX's stable extension ID. Before the host releases the restricted token, it verifies a fresh HMAC challenge against the live loopback server and its exact address; stale crash files or port relays do not pass.
...
- Raw transcripts remain local by default. VOIVOX and its MCP do not upload audio to an LLM. If a user asks Codex to read or transform MCP transcript text, that text may be processed by the model provider selected in Codex under that provider's terms and settings.
