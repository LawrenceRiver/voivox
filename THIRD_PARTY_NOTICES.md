...
Voice Vac includes or bundles the following open-source components. Their licenses remain with their respective copyright holders.
...
The explicit desktop-ASR installer creates a Python 3.12 virtual environment outside the App bundle, installs the pinned official [`qwen-asr==0.0.6`](https://pypi.org/project/qwen-asr/), and downloads the pinned [`Qwen/Qwen3-ASR-0.6B`](https://huggingface.co/Qwen/Qwen3-ASR-0.6B) snapshot. The package and model are published under the Apache License 2.0 at the time of this release. Neither the Python environment nor model weights are distributed in the Voice VAC repository or release archives; users should review the linked current terms before installing them.

Each release archive includes the Voice VAC MIT license. Extension packages include the complete Transformers.js, `@huggingface/jinja`, and ONNX Runtime license texts while those dependencies remain in the extension build. App resources include the complete license texts for the bundled MCP dependency graph, React/React DOM, and Electron. `ELECTRON_CHROMIUM_NOTICES.html` in App resources contains the Chromium and other third-party notices distributed with Electron.
