...
Voice Vac is a strong fit for **Work & Productivity**: it turns otherwise inaccessible video/app audio into local, reusable text and lets Codex operate on the result through MCP. **Apps for Your Life** is a credible alternative if the submission is framed primarily around individual creators and researchers.
...
**One sentence:** Voice Vac silently captures one user-selected media source, transcribes it with a local open model, and exposes immutable text to Codex without hijacking dictation or uploading audio.
...
2. **0:18–0:48 — Standalone extension.** Open Voice Vac, choose Fast or Quality, click once, and keep the host muted. Stop and show the local transcript plus Copy.
3. **0:48–1:18 — Automatic App connection.** Open the Voice Vac App and repeat a short capture. Show that no address/token pairing is required, tab audio remains browser-local, and only completed text is synchronized.
4. **1:18–1:48 — Codex MCP.** Ask Codex to list Voice Vac sessions, read the newest raw transcript, and produce a clearly labeled derived summary without changing the source text.
